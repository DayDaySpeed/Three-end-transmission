...
- iOS 上的 **Chrome 常无法解析 `.local` 地址**，Safari 通常可以
- 请改用 **Safari** 打开，或在 Chrome 地址栏手动输入 **局域网 IP**：`http://192.168.x.x:8787`
- 不要扫含 `.local` 的二维码，改扫 Hub 页面上 **局域网 IP** 对应的二维码

### Android 浏览器界面

- 输入框固定在屏幕底部，消息区单独滚动
- 若仍被遮挡，尝试关闭浏览器「桌面版网站」后刷新
