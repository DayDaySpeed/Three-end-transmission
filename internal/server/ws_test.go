...
func dialPeer(t *testing.T, ts *httptest.Server, id, name string) *wsPeer {
	t.Helper()
	q := url.Values{"id": {id}, "name": {name}, "platform": {"linux"}}
	...
	p := &wsPeer{t: t, id: id, conn: conn}
	// welcome 在 Register 之后发出，收到即表示已加入房间
	if _, ok := p.next("welcome", 2*time.Second); !ok {
...
func TestDirectMessageDelivery(t *testing.T) {
	...
	a := dialPeer(t, ts, uuid.NewString(), "A")
	b := dialPeer(t, ts, uuid.NewString(), "B")
	c := dialPeer(t, ts, uuid.NewString(), "C")
	...
	// 新设备只能在历史里看到群发
	d := dialPeer(t, ts, uuid.NewString(), "D")
